    ...
    <section className="my-5 sm:my-7">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-xl bg-[#B4552D]/10 text-[#B4552D] flex items-center justify-center animate-pulse">
            <Flame className="w-4 h-4 text-[#B4552D]" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-sm sm:text-base font-bold text-[#1F2430]">نبض شهر: مراسمات امشب دزفول</h2>
            ...
            <p className="text-[11px] text-[#71717A]">اطلاع‌رسانی لحظه‌ای هیئات، مساجد، دعای کمیل و محافل قرآنی</p>
          </div>
        </div>

        <span className="text-xs font-bold text-[#0E7C86] bg-[#0E7C86]/10 px-2.5 py-1 rounded-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
        {sortedEvents.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="bg-white rounded-3xl p-4 sm:p-4.5 border border-[#E0D8C8] shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between relative group"
            >
              ...
              <div className="flex items-start justify-between gap-3 mb-2.5">
                ...
                >
                  ...
                  <div>
                    <h4 className="text-xs sm:text-sm font-bold text-[#1F2430] hover:text-[#0E7C86] transition-colors line-clamp-1">
                      {event.placeName}
                    </h4>
                    <div className="flex items-center gap-1 text-[11px] text-[#71717A]">
                      <MapPin className="w-3 h-3 text-[#B4552D]" />
                ...
                {/* Time Badge */}
                <div className="flex items-center gap-1 bg-[#F7F3EC] border border-[#DDD5C5] text-[#B4552D] px-2.5 py-1 rounded-xl text-[11px] font-bold shrink-0">
              ...
              <div className="my-1.5">
                <h3 className="text-sm sm:text-base font-extrabold text-[#1F2430] leading-snug">
                  {event.title}
                </h3>

                {/* Speaker and Eulogist if present */}
                <div className="flex flex-wrap items-center gap-y-1 gap-x-3 mt-2 text-xs text-[#52525B]">
                  {event.speaker && (
                  ...
                  {event.qari && (
                    <div className="flex items-center gap-1">
                      <span className="text-emerald-700 font-bold">قاری:</span>
                      <span className="font-semibold text-[#1F2430]">{event.qari}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 my-2.5 pt-2 border-t border-[#F2ECE1]">
                {event.services.nazri && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-amber-50 text-amber-800 border border-amber-200 px-2 py-0.5 rounded-lg">
                    <Utensils className="w-3 h-3 text-amber-600" />
                    <span>دارای نذری و پذیرایی</span>
                  </span>
                )}
                {event.services.womenSection && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200 px-2 py-0.5 rounded-lg">
                    <Users className="w-3 h-3 text-emerald-600" />
                    <span>ویژه بانوان و آقایان</span>
                  </span>
                )}
                {event.services.liveStream && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-red-50 text-red-800 border border-red-200 px-2 py-0.5 rounded-lg">
                    <Radio className="w-3 h-3 text-red-600 animate-pulse" />
                    <span>پخش زنده</span>
                  </span>
                )}
                {event.services.shovadoonActive && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-stone-100 text-stone-800 border border-stone-300 px-2 py-0.5 rounded-lg">
              ...
              <div className="flex items-center gap-2 mt-1 relative">
                ...
                <div className="flex-1 relative">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 bg-[#0E7C86] hover:bg-[#0b636b] text-white text-xs sm:text-sm font-bold py-2.5 px-3 rounded-xl shadow-xs transition-all active:scale-[0.98]"
                  >
                  ...
                  {isRoutingMenuOpen && (
                    <div className="absolute bottom-full mb-1 right-0 left-0 bg-white rounded-2xl p-2 border border-[#DDD5C5] shadow-xl z-30 animate-fadeIn">
                      <div className="text-[11px] font-bold text-[#71717A] px-2 py-1 border-b border-[#F2ECE1] mb-1">
                      ...
                      <div className="grid grid-cols-3 gap-1 text-center">
                        <a
                          ...
                          className="p-1.5 rounded-xl hover:bg-[#F7F3EC] text-xs font-bold text-[#1F2430] flex flex-col items-center gap-1 transition-colors"
                        >
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-xs sm:text-sm font-bold border transition-all active:scale-[0.98] ${
                    isReminderSet
                      ...
                      : 'bg-[#F7F3EC] text-[#52525B] border-[#DDD5C5] hover:bg-white hover:border-[#0E7C86] hover:text-[#0E7C86]'
                  }`}
                  title={isReminderSet ? 'یادآوری تنظیم شد' : 'یادآوری کن'}
                >
                  {isReminderSet ? (
                    <>
                      <BellRing className="w-4 h-4 text-amber-600 fill-amber-600/20" />
